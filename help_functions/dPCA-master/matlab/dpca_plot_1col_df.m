function Zfull = dpca_plot_1col_df(Xfull, W, V, plotFunction, varargin)
   ...
   length(unique(options.whichMarg)) <= 4 && length(unique(options.whichMarg)) > 1
    ...
    for i=1:length(margRowSeq)
        if ~isempty(options.componentsSignif) && margRowSeq(i) ~= options.timeMarginalization
            ...
            moreComponents = find(options.whichMarg == margRowSeq(i), 1); % find(options.whichMarg == margRowSeq(i), 3);
        end
    ...
    if isempty(options.whichMarg)
        ...
        componentsToPlot = 1:12; % CHANGED FROM 1:12
    else
...
%$ Edit figure size and properties
myFig = figure('Units','centimeters', 'Position', [0 0 21.5 14]);
% myFig = figure('Position', [0 0 1800*0.4 1000*0.4]);
...
for c = 1:length(componentsToPlot)
    ...
    if ismember(subplots(c), [6])
        ylabel('Normalized firing rate (Hz)')
    end

    if ismember(subplots(c), [2 6 10 14])
        if subplots(c) == 14 % subplots(c) == 2 || 
            xlabel('Time (s)')
            set(gca,'XTick',[0,1,2],'XLim',[options.time(1), options.time(end)]) % 'XLim',[-0.5,2.0])
        else
            set(gca, 'XTickLabel', [],'XTick',[0,1,2],'XLim',[options.time(1), options.time(end)])
        end
        
    elseif ismember(subplots(c), [13 14 15 16])
        xlabel('Time (s)')
        set(gca, 'YTickLabel', [],'XTick',[0,1,2],'XLim',[options.time(1), options.time(end)])
    else
        set(gca, 'XTickLabel', [])
        set(gca, 'YTickLabel', [])
    end
...
if false %~isempty(options.explainedVar)
    axBar = subplot(4,4,5);
    hold on
...
fontsize(myFig,8,'points'); % This is changing the subplot sizes! WHY?!?!
fontname(myFig,"Arial");
% fontsize(myFig,'default');
...
for pl = intersect(3:4:16, subplots)
    subplot(4,4,pl)
    pos = get(gca, 'Position');
...
% pie chart
if false %~isempty(options.explainedVar)
    axes('position', [0.21 0.6 0.1 0.1])
